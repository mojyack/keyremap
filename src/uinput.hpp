...
inline auto get_uinput_device_name(const char* const path) -> Result<std::string> {
...
inline auto enumerate_devices() -> std::vector<InputDeviceInfo> {
...
inline auto open_uinput_device(const char* const path, bool grab = true) -> Result<int> {
    const auto fd = open(path, O_RDONLY);
    if(fd == -1) {
        return Error(build_string("open(", path, ") failed: ", errno));
    }
...
inline auto create_virtual_device(const std::vector<Device>& parent_devices, const char* const device_name) -> Result<int> {
    const auto fd = open("/dev/uinput", O_WRONLY | O_NONBLOCK);
    if(fd == -1) {
        return Error(build_string("open(/dev/uinput) failed: ", errno));
    }

    for(const auto& pd : parent_devices) {
        const auto pfd    = pd.fd;
